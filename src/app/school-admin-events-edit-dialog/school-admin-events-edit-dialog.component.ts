...
export class SchoolAdminEventsEditDialogComponent implements OnInit {
  constructor(
    ...
    public dialogRef: MatDialogRef<SchoolAdminEventsEditDialogComponent>,
  ...
  onSubmit() {
    if (!!this.data.eventId) {
      ...
      // Create new user
      this.afs
        .collection<Event>(`schools/${this.data.schoolId}/events`)
        .add(this.event)
        .then(() =>
          this.snackbar.open("Added new event", null, { duration: 1000 })
        )
        .catch(() =>
          this.snackbar.open("Failed to add event", null, { duration: 1000 })
        );
    }
