...
export class SchoolAdminStudentsDataSource extends DataSource<UserDetailsId> {
  ...
  async getData() {
    console.log("Attemping to get data");
    try {
      const user: User = await this.authService.getUser().pipe(first()).toPromise();
      console.log("Got user");
      const userDetails = await this.afs.collection<UserDetailsId>(`schools/${user.school.id}/user-details`).snapshotChanges().pipe(map(actions => actions.map(a => {
